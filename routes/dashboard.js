...
router.get('/expiring', async (req, res) => {
  try {
    ...
    WHERE pb.expiry_date BETWEEN date_trunc('month', CURRENT_DATE)
        AND (date_trunc('month', CURRENT_DATE) + INTERVAL '6 months' - INTERVAL '1 day')
        AND pb.stock > 0
    ...
    const sql = `
      ...
      diskon_calc AS (
        SELECT 
          ...
          -- Omzet hasil dari diskon (tambahan untuk tampilan)
          CASE 
            WHEN er.recommendation ILIKE '%diskon%' THEN 
