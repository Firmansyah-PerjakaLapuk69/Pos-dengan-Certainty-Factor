...
router.get('/receipt/:id', async (req, res) => {
    ...
    try {
        ...
        items.forEach((item, index) => {
            ...
            const discountDisplay = `${rupiah(totalDiscount)} (${discountPercentage.toFixed(1)}%)`;
            ...
            doc.text(`(${discountPercentage.toFixed(1)}%)`, 420, doc.y, { width: 80, align: 'right' });
            doc.text(rupiah(item.subtotal), 500, y, { width: 90, align: 'right' });
            doc.moveDown(0.5);
        });
